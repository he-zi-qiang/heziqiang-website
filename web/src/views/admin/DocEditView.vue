...
import type { AboutDocSource, CvDocSource, HomeDocSource, SectionsDoc, SiteDoc } from '@/api/types'
...
<template>
  ...
  <p class="admin-sub">
    这些是不属于任何栏目的固定文案。值里可以写行内 Markdown 与少量 HTML
    （<code>&lt;br&gt;</code>、<code>&lt;span class="light"&gt;</code> 会按站点样式渲染）。
  </p>
  ...
  <form v-else-if="doc" @submit.prevent="save">
    ...
    <template v-else-if="docKey === 'sections'">
      <div v-for="(meta, key) in sections" :key="key" class="section-block">
        <h3>{{ key }}</h3>
        <div class="field"><label>标题</label><input v-model="meta.title" type="text" /></div>
        <div class="field">
          <label>副标题</label>
